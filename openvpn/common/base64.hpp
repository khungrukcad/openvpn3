...
namespace openvpn {
  ...
  public:
	...
	{
	  unsigned int marker;
	  const unsigned int val = token_decode(p, std::min(endp - p, ssize_t(4)), marker);
  ...
  private:
    ...
    unsigned int token_decode(const char *token, const ssize_t len, unsigned int& marker) const
    {
      size_t i;
      unsigned int val = 0;
